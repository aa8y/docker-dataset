...
* `geonames`: schema authored in-repo (`mysql/scripts/geonames/schema.sql`); the tab-separated export is bulk-loaded at start with server-side `LOAD DATA INFILE`. The table and the load pin `utf8mb4` explicitly (the base image creates the database as `utf8mb3`, which cannot hold the non-BMP place names in `alternatenames`), and the nullable integer columns are `NULLIF`ed so a blank field is `NULL` rather than `0`. Counts drift as GeoNames rebuilds the dump daily (recorded as floors).
* `openflights`: schema authored in-repo (`mysql/scripts/openflights/schema.sql`, 3 tables); the three data files are bulk-loaded at start with server-side `LOAD DATA INFILE`, whose default escape character maps the files' `\N` to `NULL`. No foreign keys (`routes` deliberately keeps dangling airport/airline references). Counts drift as upstream refreshes the files (recorded as floors).
...
* `geonames`: schema authored in-repo (`sqlite/scripts/geonames/schema.sql`); the tab-separated export is bulk-loaded at build time with the sqlite3 CLI's `.import` dot-command in `ascii` mode (no quote processing, so a double quote in a place name is a literal character), then blank integer fields are normalized to `NULL`. Counts drift as GeoNames rebuilds the dump daily (recorded as floors).
* `openflights`: schema authored in-repo (`sqlite/scripts/openflights/schema.sql`, 3 tables); the three data files are bulk-loaded at build time with the sqlite3 CLI's `.import` dot-command, then their `\N` markers are rewritten to `NULL` (the CSV importer has no NULL sentinel). No foreign keys (`routes` deliberately keeps dangling airport/airline references). Counts drift as upstream refreshes the files (recorded as floors).

...
* `employees`: datacharmer's canonical *large* sample (6 tables + 2 views; 300,024 employees and 2,844,047 salary rows — by far the heaviest SQLite image). Upstream ships no SQLite port, but its PostgreSQL one is near dialect-free, so `sqlite/scripts/employees/transform` uses that schema with the three rewrites SQLite forces — the `DROP`/`CREATE DATABASE` + `\connect` header dropped, the multi-table `DROP TABLE ... CASCADE` dropped, `CREATE OR REPLACE VIEW` → `CREATE VIEW` — and appends the eight data dumps unchanged inside one transaction. Unlike the datasets that go through the shared `pgsql` hook, this one **keeps** its primary, foreign and unique keys (they are inline `CREATE TABLE` constraints, which SQLite accepts) as well as both views. Row counts are exact and match the MySQL and PostgreSQL tags.
